...
function DocumentsTab({
  ...
  const groups: Array<{ label: string; source: DocumentRecord['uploadedBy'] }> = [
    { label: 'Patient Uploads', source: 'patient' },
    { label: 'Clinic Uploads', source: 'clinic' },
    { label: 'Staff Attachments', source: 'staff' },
  ];
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {documents.length === 0 ? (
          ...
          <div className="mt-4 space-y-5">
            {groups.map((group) => {
              const docs = documents.filter((document) => document.uploadedBy === group.source);
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <h3 className="text-sm font-semibold text-slate-900">Attach Staff Document</h3>
        <p className="mt-1 text-sm text-slate-500">
          Simulate adding a document to the staff cockpit without changing patient portal behavior.
        </p>
        <form onSubmit={handleUpload} className="mt-4 space-y-3">
          <label className="block space-y-1.5">
            ...
            <input
              ...
              placeholder="e.g. Financial clearance note"
          ...
          <button
            ...
            className="inline-flex w-full items-center justify-center gap-1.5 rounded-xl bg-[#1a66cc] px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-[#1558ad] disabled:cursor-not-allowed disabled:bg-slate-300"
          >
